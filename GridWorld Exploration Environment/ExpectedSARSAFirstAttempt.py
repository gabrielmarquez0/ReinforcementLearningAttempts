...
def ExpSARSA(episodesN, startState, endState, policy, updateValueFunction, updateState, updateReward, Q):
    records = {}
...
def main():
    ...
    startState = 0
    endState = 1
    records = ExpSARSA(episodesN,
       startState,
       endState,
       lambda x,y: eGreedy(epsilon, x, y),
       lambda v,w,x,y,z: updateValueFunctionExpSARSA(alpha, gamma, v, w, x, y, z),
       lambda x,y: updateStateExpSARSA(x,y, transition),
       lambda x,y: updateRewardExpSARSA(x,y, reward),
       Q)
